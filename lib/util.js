...
//
// Resolves with the properties on 'properties', rejects on the first 'error'.
// Critically, it also rejects after `timeoutMs` if the session emits neither:
// a Modbus session can stall without ever producing an event - for example a TCP
// connect that the peer silently drops, which the OS only gives up on after
// minutes - and an unsettled promise leaves the pairing view spinning forever
// with nothing to show the user.
//
// On failure the session is disconnected so its socket can't linger, which also
// frees the single Modbus slot most SMA devices expose. On success the session
// closes itself (autoClose), so it is left alone.
exports.waitForProperties = function (smaSession, timeoutMs = PROPERTIES_TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    ...
    const onError = (err) => {
      ...
      cleanup();
      teardown();
      reject(err instanceof Error ? err : new Error(exports.formatError(err)));
    };
    ...
    timer = setTimeout(() => {
      ...
      cleanup();
      teardown();
      reject(new Error(`Timed out after ${Math.round(timeoutMs / 100) / 10}s waiting for the device to report its properties`));
    }, timeoutMs);
  });
}
