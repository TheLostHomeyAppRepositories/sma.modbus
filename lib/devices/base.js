...
class Base extends HomeyEventEmitter {
  ...
  constructor(deviceRegistryHandler, options = {}) {
    ...
    this._logMessage('INFO', `Initializing with options: host=${this.options.host}, port=${this.options.port}, refreshInterval=${this.options.refreshInterval}, timeout=${this.options.timeout}s (effective ${this.#resolveTimeoutMs()}ms), autoClose=${this.options.autoClose}`);
  ...
  async #initializeDevice() {
    ...
    try {
      if (!this.options.host || !util.validateIPaddress(this.options.host)) {
        const errMsg = `Invalid IP address '${this.options.host}'`;
        this._logMessage('INFO', errMsg);
        this._emitError(new Error(errMsg));
        return;
      }
      ...
      if (!available) {
        const errMsg = `Port '${this.options.port}' on IP Address '${this.options.host}' is NOT reachable`;
        this._logMessage('INFO', errMsg);
        this._emitError(new Error(errMsg));
        return;
      }

      // The validator above just opened a Modbus session and dropped it. Most SMA
      // devices accept only one Modbus TCP client and are slow to release the
      // slot, so reconnecting immediately can be refused - or silently ignored,
      // which is worse because a dropped SYN only surfaces after the OS connect
      // timeout. Give the device the same settle time the reconnect path uses.
