    ...
    async onRepair(session, device) {
        ...
        const withSuspendedConnection = async (operation) => {
            ...
            try {
                ...
                // If session.done() already applied connection settings,
                // onSettings has resumed the device and this becomes a no-op.
        ...
        session.setHandler('showView', async (view) => {
            ...
            if (view === 'loading') {
                await withSuspendedConnection(async () => {
                    ...
                    // Discovery verifies candidates with temporary Modbus sessions;
                    // pause the owning device so it cannot occupy the same slot.
                    const discoveryQuery = new InverterDiscovery({
                        port: this.homey.settings.get('port'),
                        device: this
                    });
